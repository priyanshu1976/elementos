...
export default function RegistrationForm({
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const validMembers = formData.members.filter((member): member is TeamMember => 
        member !== null && member.name !== "" && member.email !== "" && member.phone !== ""
      );
      ...
      const teamDocRef = doc(collection(db, eventCollection), formData.teamName);
  ...
  const handleChange = (
    ...
    if (type === "leader" && field !== "teamName") {
      ...
      if (!value && !Object.values(newMembers[index] as TeamMember).some(v => v)) {
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            <div className="space-y-6">
              <input
                ...
                onChange={(e) => handleChange("team", null, "teamName", e.target.value)}
              />

              <div className="bg-black/40 border border-[#FF0F5B]/20 p-6 rounded-lg space-y-4">
                <h3 className="text-[#FF0F5B] text-xl font-mono">SQUAD LEADER</h3>
                {["name", "email", "phone"].map((field) => (
                  <input
                    ...
                    required
                    className="w-full bg-black/50 text-white px-4 py-3 rounded-lg border border-[#FF0F5B]/30 focus:outline-none focus:border-[#FF0F5B] focus:ring-1 focus:ring-[#FF0F5B] transition-colors placeholder:text-gray-500"
                    onChange={(e) => handleChange("leader", null, field as keyof TeamMember, e.target.value)}
                  />
                ))}
              </div>
                ...
                >
                  ...
                  {["name", "email", "phone"].map((field) => (
                    <input
                      ...
                      placeholder={field.charAt(0).toUpperCase() + field.slice(1)}
                      className="w-full bg-black/50 text-white px-4 py-3 rounded-lg border border-[#FF0F5B]/30 focus:outline-none focus:border-[#FF0F5B] focus:ring-1 focus:ring-[#FF0F5B] transition-colors placeholder:text-gray-500"
                      onChange={(e) => handleChange("member", index, field as keyof TeamMember, e.target.value)}
                    />
                  ))}
                </motion.div>
              ))}
